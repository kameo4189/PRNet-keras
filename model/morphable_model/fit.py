...
def fit_points(x, X_ind, model, n_sp, n_ep, max_iter = 4):
    ...
    for i in range(max_iter):
        ...
        #----- estimate pose
        P = mesh_transform.estimate_affine_matrix_3d22d(X.T, x.T)
        s, R, t = common_methods.P2sRt(P)
        rx, ry, rz = common_methods.matrix2angle(R)
...
def fit_points_for_show(x, X_ind, model, n_sp, n_ep, max_iter = 4):
    ...
    s = 4e-04
    R = common_methods.angle2matrix([0, 0, 0])
    t = [0, 0, 0]
    lsp = []; lep = []; ls = []; lR = []; lt = []
    for i in range(max_iter):
        X = shapeMU + shapePC.dot(sp) + expPC.dot(ep)
